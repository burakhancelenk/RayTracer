...
Vector3 Render::render_ray(const Scene& scene, const Ray& ray, int depth)
{
	...
	for (const LightPtr& light : scene.get_lights())
	{
		...
		float lightDistance;
		light->at(hit.point, lightDirection, incomingLightPower, lightDistance);
		...
		{
			// First, we check whether the light source is visible
	...
	if (depth < settings.maxDepth && reflectionCoefficient > 0)
	{
		...
		color = color + reflected * abs(reflectionCoefficient-1)
				* incoming * reflectionCoefficient;
	}
